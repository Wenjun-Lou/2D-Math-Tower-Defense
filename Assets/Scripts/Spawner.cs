...
public class Spawner : MonoBehaviour
{
    ...
    [SerializeField] private ObjectPooler orcGruntPool; 
    [SerializeField] private ObjectPooler orcRunnerPool;
    [SerializeField] private ObjectPooler orcBerserkerPool;
    [SerializeField] private ObjectPooler orcBrutePool;
    [SerializeField] private ObjectPooler orcWarlordPool;
    ...
    private void Awake()
    {
        ...
        {
            { EnemyType.Orc_Grunt, orcGruntPool},
            { EnemyType.Orc_Runner, orcRunnerPool},
            { EnemyType.Orc_Berserker, orcBerserkerPool},
            { EnemyType.Orc_Brute, orcBrutePool},
            { EnemyType.Orc_Warlord, orcWarlordPool},
        };
    ...
    private void SpawnEnemy()
    {
        if (_poolDictionary.TryGetValue(CurrentWave.enemyType, out var pool))
        {
            ...
            float healthMultiplier = 1f + (_waveCounter * 0.3f); // +30% per wave
    ...
    {
        _isGamePlayScene = scene.name != "MainMenu" && scene.name != "LevelSelect";
        ResetWaveState();

        if (!_isGamePlayScene) return;

        _isEndlessMode = scene.name == "EndlessMode";
